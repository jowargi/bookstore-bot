...
def insert_row(db_path, table_name, **values):
    ...
    with sqlite3.connect(db_path) as conn:
        cursor = conn.cursor()
        cursor.execute(
            f"INSERT INTO {table_name}({columns}) VALUES({placeholders})", params
        )
        conn.commit()
...
def update_row(db_path, table_name, condition_column, condition_value, **updates):
    ...
    with sqlite3.connect(db_path) as conn:
        cursor = conn.cursor()
        cursor.execute(
            f"UPDATE {table_name} SET {set_clause} WHERE {condition_column} = ?", params
        )
        conn.commit()


def fetch_data(
    db_path, table_name, condition_column=None, condition_value=None, limit=None
):
    with sqlite3.connect(db_path) as conn:
        cursor = conn.cursor()
        ...
        if condition_column and condition_value:
            cursor.execute(
                f"SELECT * FROM {table_name} WHERE {condition_column} = ?",
                (condition_value,),
            )
